# Recurrent network implementation is adapted from https://github.com/declare-lab/conv-emotion/
...
class MemoryBankCell(nn.Module):
    ...
    def __init__(self, D_sense, D_txt, D_hist, D_participant, D_r, D_i, D_rt, listener_state=False,
                 context_attention='simple', D_a=100, dropout=0.5, emo_gru=True):
        super(MemoryBankCell, self).__init__()
        ...
        self.g_cell = nn.GRUCell(D_sense + D_participant + D_r, D_hist)
        self.p_cell = nn.GRUCell(D_txt + D_hist, D_participant)
        self.r_cell = nn.GRUCell(D_sense + D_txt + D_hist, D_r)
        self.i_cell = nn.GRUCell(D_txt + D_participant, D_i)
        self.e_cell = nn.GRUCell(D_sense + D_participant + D_r + D_i, D_rt)
        
        
        self.emo_gru = emo_gru
        self.listener_state = listener_state
        if listener_state:
            self.pl_cell = nn.GRUCell(D_txt + D_hist, D_participant)
            self.rl_cell = nn.GRUCell(D_sense + D_txt + D_hist, D_r)
        ...
        if context_attention=='simple':
            self.attention = SimpleAttention(D_hist)
        else:
            self.attention = MatchingAttention(D_hist, D_sense, D_a, context_attention)
    ...
    def forward(self, U, x1, x2, x3, o1, o2, qmask, g_hist, q0, r0, i0, e0):
        """
        U -> batch, d_comet
        x1, x2, x3, o1, o2 -> batch, d_comet
        qmask -> batch, party
        g_hist -> t-1, batch, D_hist
        q0 -> batch, party, D_participant
        e0 -> batch, self.D_rt
        """
        ...
        g_ = self.g_cell(torch.cat([U, q0_sel, r0_sel], dim=1),
                torch.zeros(U.size()[0], self.D_hist).type(U.type()) if g_hist.size()[0] == 0 else
                g_hist[-1])
        # g_ = self.dropout(g_)
        
        ## context ##
        if g_hist.size()[0]==0:
            c_ = torch.zeros(U.size()[0], self.D_hist).type(U.type())
            alpha = None
        else:
        ...
        rs_ = self.r_cell(U_r_c_.contiguous().view(-1, self.D_sense + self.D_txt + self.D_hist),
        ...
        qs_ = self.p_cell(es_c_.contiguous().view(-1, self.D_txt + self.D_hist),
                          q0.view(-1, self.D_participant)).view(U.size()[0], -1, self.D_participant)
        # qs_ = self.dropout(qs_)
        

        if self.listener_state:
            ## listener external state ##
            U_ = U.unsqueeze(1).expand(-1,qmask.size()[1],-1).contiguous().view(-1, self.D_sense)
            er_ = o2.unsqueeze(1).expand(-1, qmask.size()[1], -1).contiguous().view(-1, self.D_txt)
            ...
            es_ = o1.unsqueeze(1).expand(-1, qmask.size()[1], -1).contiguous().view(-1, self.D_txt)
            ss_ = self._select_parties(qs_, qm_idx).unsqueeze(1).\
                    expand(-1, qmask.size()[1], -1).contiguous().view(-1, self.D_participant)
            es_ss_ = torch.cat([es_, ss_], 1)
            ql_ = self.pl_cell(es_ss_, q0.view(-1, self.D_participant)).view(U.size()[0], -1, self.D_participant)
        ...
        is_ = self.i_cell(i_q_.contiguous().view(-1, self.D_txt + self.D_participant),
        ...
        e0 = torch.zeros(qmask.size()[0], self.D_rt).type(U.type()) if e0.size()[0] == 0\
                else e0
...
class MemoryBank(nn.Module):
    ...
    def __init__(self, D_sense, D_txt, D_hist, D_participant, D_r, D_i, D_rt, listener_state=False,
        ...
        self.dialogue_cell = MemoryBankCell(D_sense, D_txt, D_hist, D_participant, D_r, D_i, D_rt,
                            listener_state, context_attention, D_a, dropout, emo_gru)

    def forward(self, U, x1, x2, x3, o1, o2, qmask):
        """
        U -> seq_len, batch, d_comet
        x1, x2, x3, o1, o2 -> seq_len, batch, D_txt
        qmask -> seq_len, batch, party
        """

        g_hist = torch.zeros(0).type(U.type()) # 0-dimensional tensor
        q_ = torch.zeros(qmask.size()[1], qmask.size()[2], self.D_participant).type(U.type()) # batch, party, D_participant
        ...
        e_ = torch.zeros(0).type(U.type()) # batch, D_rt
        e = e_
        ...
        return e, alpha # seq_len, batch, D_rt


class Kamuh(nn.Module):

    def __init__(self, d_comet, D_txt, D_hist, D_participant, D_r, D_i, D_rt, D_h, D_a=100, n_classes=7, listener_state=False,
                 context_attention='simple', dropout_rec=0.5, dropout=0.1, emo_gru=True, mode1=0, norm=0, residual=False):

        super(Kamuh, self).__init__()
        ...
        norm_train = True
        self.norm1a = nn.LayerNorm(d_comet, elementwise_affine=norm_train)
        self.norm1b = nn.LayerNorm(d_comet, elementwise_affine=norm_train)
        self.norm1c = nn.LayerNorm(d_comet, elementwise_affine=norm_train)
        self.norm1d = nn.LayerNorm(d_comet, elementwise_affine=norm_train)

        self.norm3a = nn.BatchNorm1d(d_comet, affine=norm_train)
        self.norm3b = nn.BatchNorm1d(d_comet, affine=norm_train)
        self.norm3c = nn.BatchNorm1d(d_comet, affine=norm_train)
        self.norm3d = nn.BatchNorm1d(d_comet, affine=norm_train)

        self.dropout   = nn.Dropout(dropout)
        self.dropout_rec = nn.Dropout(dropout_rec)
        self.cs_rnn_f = MemoryBank(D_h, D_txt, D_hist, D_participant, D_r, D_i, D_rt, listener_state,
                                       context_attention, D_a, dropout_rec, emo_gru)
        # self.cs_rnn_r = MemoryBank(D_h, D_txt, D_hist, D_participant, D_r, D_i, D_rt, listener_state,
        #                                context_attention, D_a, dropout_rec, emo_gru)
        self.sense_gru = nn.GRU(input_size=D_txt, hidden_size=D_txt//2, num_layers=1, bidirectional=True)
        self.matchatt = MatchingAttention(2*D_rt,2*D_rt,att_type='general2')
        self.linear     = nn.Linear(2*D_rt, D_h)
